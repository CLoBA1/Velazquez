...
create_html = """                {{-- Card: Gestión de Precios Unificado --}}
                <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden" x-data="{
                    ...
                    <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                        ...
                        <div class="flex flex-wrap items-center gap-4 bg-white px-4 py-2 flex-col md:flex-row rounded-lg border border-slate-200 shadow-sm">
                    ...
                    <div class="p-8 space-y-10">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wide border-b border-slate-200 pb-2 mb-6">1. Precio de la Unidad Base</h3>
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <input type="number" step="any" x-model="base_margin" @input="updateBasePrice()" placeholder="Ej. 30" class="w-full pl-3 pr-8 py-2.5 rounded-xl border-emerald-300 text-sm font-bold text-emerald-700 bg-emerald-50 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all shadow-sm">
                        ...
                        <div>
                            <div class="flex items-center justify-between border-b border-slate-200 pb-2 mb-6">
                                <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wide">2. Presentaciones Adicionales</h3>
                                <button type="button" @click="addUnit()" class="inline-flex items-center gap-1.5 text-sm font-bold text-blue-600 hover:text-blue-800 hover:bg-blue-50 px-3 py-1 rounded-lg transition-colors">
                            ...
                            <div class="space-y-4">
                                <template x-for="(unit, index) in units" :key="index">
                                    <div class="p-5 rounded-2xl border border-slate-200 bg-slate-50/80 relative group hover:border-blue-300 hover:bg-white transition-colors shadow-sm">
                                        
                                        <button type="button" @click="removeUnit(index)" class="absolute top-4 right-4 text-slate-400 hover:text-red-500 opacity-50 group-hover:opacity-100 transition-opacity bg-white hover:bg-red-50 rounded-lg p-1">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>

                                        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 pr-8">
                                            ...
                                            <div class="md:col-span-2">
                                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Equivale A</label>
                                                <div class="relative">
                                                    <input type="number" step="0.0001" :name="'units['+index+'][conversion_factor]'" x-model="unit.conversion_factor" @input="updateUnitCost(unit)" required class="w-full pr-8 py-2.5 rounded-xl border-slate-300 text-sm font-bold text-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 shadow-sm">
                                                    <span class="absolute right-3 top-2.5 text-slate-400 text-xs font-bold leading-5">U.Base</span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="md:col-span-2">
                                                ...
                                                <div class="relative">
                                                    <input type="number" step="any" x-model="unit.margin" @input="updateUnitPrice(unit)" class="w-full pl-3 pr-8 py-2.5 rounded-xl border-emerald-300 text-sm font-bold text-emerald-700 bg-emerald-50 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all shadow-sm">
                                                    <span class="absolute right-3 top-2.5 text-emerald-500 font-bold">%</span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="md:col-span-3">
                                                ...
                                                {{-- Hidden fields to satisfy current validation --}}
...
def replace_in_file(filepath, new_block):
    ...
    for i, line in enumerate(lines):
        # Look for the start block based on multiple possibilities due to varying comments
        if 'Card: Gestión de Precios' in line or 'Card: Gestin de Precios Unificado' in line:
            # Only start if the next line is the beginning of the rounded-2xl div
            if 'rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden' in lines[i+1]:
                start_idx = i
                break
            elif 'rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden' in line:
                start_idx = i - 1
                break
    
    # If the above failed, try searching for the div itself
    if start_idx == -1:
         for i, line in enumerate(lines):
