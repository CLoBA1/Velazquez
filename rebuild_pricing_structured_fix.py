...
edit_html = create_html.replace(
    "cost: {{ Js::from(old('cost_price', 0)) }},", "cost: {{ Js::from(old('cost_price', $product->cost_price)) }},"
).replace(
    "tax_percent: {{ Js::from(old('taxes_percent', 0)) }},", "tax_percent: {{ Js::from(old('taxes_percent', $product->taxes_percent)) }},"
).replace(
    "base_public_price: {{ Js::from(old('public_price', '')) }},", "base_public_price: {{ Js::from(old('public_price', $product->public_price)) }},"
).replace(
    "units: {{ Js::from(old('units', [])) }}", "units: {{ Js::from(old('units', $product->units->toArray())) }}"
)
...
def replace_in_file(filepath, new_block):
    ...
    # Find the bounds from `{{-- Card: Gestión de Precios Unificado` to `{{-- Card: Imagen --}}`
    start_match = re.search(r'\{\{-- Card: Gesti[oó]n de Precios Unificado', content)
    if not start_match:
        # fallback
        start_match = re.search(r'<div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden" x-data="\{', content)
    
    end_match = re.search(r'\{\{-- Card: Fotografía --\}\}|\{\{-- Card: Imagen --\}\}', content)
    
    if start_match and end_match:
        start_idx = start_match.start()
        end_idx = end_match.start()
        
        # Replace the unified price section with our new_block
        new_content = content[:start_idx] + new_block + "\n                " + content[end_idx:]
        
        # Now remove `addUnit` and `removeUnit` from the root form element
        new_content = re.sub(
        ...
        # Append Alpine.data script before @endsection
        if "Alpine.data('priceControl'" not in new_content:
            new_content = new_content.replace('@endsection', alpine_script + '\n@endsection')
...
replace_in_file('c:/dev/ferreteria/resources/views/admin/products/create.blade.php', create_html)
replace_in_file('c:/dev/ferreteria/resources/views/admin/products/edit.blade.php', edit_html)
