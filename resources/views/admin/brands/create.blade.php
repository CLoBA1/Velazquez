...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Nueva marca</h1>
            <p class="text-slate-500 mt-1">Registra un nuevo fabricante o marca.</p>
        </div>
    ...
    <div class="max-w-3xl mx-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.brands.store') }}" class="space-y-6" enctype="multipart/form-data">
            @csrf

            <div class="space-y-6">
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Nombre de la Marca</label>
                    <input name="name" value="{{ old('name') }}" required placeholder="Ej: Truper, Makita, DeWalt"
