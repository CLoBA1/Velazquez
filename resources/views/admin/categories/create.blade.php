...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Nueva categoría</h1>
            <p class="text-slate-500 mt-1">Crea una nueva categoría y asignala a una familia.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-6">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select name="family_id" required
                        ...
                        @foreach($families as $f)
                            <option value="{{ $f->id }}" @selected(old('family_id') == $f->id)>
                                {{ $f->name }} ({{ $f->code }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input name="name" value="{{ old('name') }}" required placeholder="Ej: Taladros Inalámbricos"
