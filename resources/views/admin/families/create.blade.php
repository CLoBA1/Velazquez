...
@section('content')
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Nueva familia</h1>
            <p class="text-slate-500 mt-1">Registra una nueva familia para clasificar productos.</p>
        </div>
    ...
    <div class="max-w-3xl mx-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.families.store') }}" class="space-y-6">
            @csrf

            <div class="space-y-6">
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Nombre de la Familia</label>
                    <input name="name" value="{{ old('name') }}" required placeholder="Ej: Herramientas Eléctricas"
                    ...
                    <p class="text-xs text-slate-500 mt-2">Nombre principal de la familia (categoría padre).</p>
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Código (Prefijo)</label>
                    <input name="code" value="{{ old('code') }}" required placeholder="Ej: HRE"
                    ...
                    <p class="text-xs text-slate-500 mt-2">Prefijo utilizado para generar códigos internos.</p>
                </div>
            </div>
