    ...
    <div class="relative bg-slate-900 rounded-3xl p-8 mb-8 overflow-hidden shadow-2xl">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                ...
                <p class="text-slate-400 mt-2 text-lg font-medium pl-6">Nivel superior de clasificación de productos.</p>
            </div>
            ...
            <table class="min-w-full divide-y divide-slate-100">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Código
                            Prefijo</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($families as $f)
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            <td class="px-6 py-4 text-sm text-slate-500 font-mono font-medium">{{ $f->id }}</td>
                            <td class="px-6 py-4">
                                <span class="font-bold text-slate-700 text-base">{{ $f->name }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-xs font-bold text-slate-600 font-mono ring-1 ring-slate-600/10">
                                    {{ $f->code }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.families.edit', $f) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.families.destroy', $f) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete(this.closest('form'), '¿Eliminar esta familia?', 'Eliminar familia')"
                    ...
                    @empty
                        <tr>
                            <td class="px-6 py-12 text-center text-slate-400" colspan="5">
                                <p class="font-medium">No hay familias registradas.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
