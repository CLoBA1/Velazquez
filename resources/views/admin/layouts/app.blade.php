...
<body class="bg-gray-50 text-gray-900" x-data="{ sidebarOpen: false }">
    <div class="min-h-screen bg-gray-100 flex flex-col md:flex-row">
        ...
        <div x-show="sidebarOpen" @click="sidebarOpen = false"
            ...
            class="fixed inset-0 bg-slate-900/80 z-40 md:hidden backdrop-blur-sm" style="display: none;"></div>

        <!-- Sidebar -->
        <aside
            class="fixed inset-y-0 left-0 w-64 bg-dark text-white flex flex-col shadow-2xl transition-transform duration-300 z-50 transform md:translate-x-0"
            :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
            <div class="h-20 flex items-center justify-center border-b border-slate-800 bg-slate-950 relative p-4">
                <div class="flex flex-col items-center">
                    <span
                        class="text-sm font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-secondary to-yellow-200 text-center leading-tight">
                        FERRETERÍA VELÁZQUEZ
                    </span>
                    <span class="text-[10px] text-slate-500 uppercase tracking-widest mt-0.5">Panel
                        Administrativo</span>
                </div>
                <!-- Close Button (Mobile) -->
            ...
            <nav class="flex-1 overflow-y-auto py-6 px-3 space-y-1">
                ...
                @foreach([
                        ...
                        ['route' => 'admin.banners.index', 'label' => 'Banners Carrusel', 'icon' => 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z'],
               ...
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                ...
                @if(auth()->user()->isAdmin())
                    ...
                    <a href="{{ route('admin.users.index') }}" 
                        class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transiti
                        on-colors
                ...
                <p class
                       ="px-3 text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2 mt-6">Sistema</p>
            ...
            <div class="p-4 border-t border-slate-800 bg-slate-900/50">
                <div class="flex items-center gap-3 mb-4">
                    <div class="h-10 w-10
                        rounded-full bg-secondary flex items-center justify-center text-dark font-bold text-lg shadow-lg shadow-yellow-500/20">
