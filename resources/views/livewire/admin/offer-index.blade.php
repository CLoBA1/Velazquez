                ...
                @forelse($offers as $offer)
                    <tr wire:key="offer-{{ $offer->id }}" class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-lg bg-gray-100 p-1 border border-gray-200 flex-shrink-0">
                                    @if($offer->image_url)
                                        <img src="{{ $offer->image_url }}" class="w-full h-full object-contain mix-blend-multiply">
                                    @else
                        ...
                        <td class="px-6 py-4 text-center">
                            <span class="font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @if($discountPercent <= 0)
                                <div class="inline-flex flex-col items-center">
                                    <span class="font-bold text-slate-400 text-xs bg-slate-100 px-2 py-0.5 rounded">Sin Descuento</span>
                            ...
                            @else
                                <div class="inline-flex flex-col items-center">
                                    ...
                                    <span class="text-[10px] text-slate-400">Ahorras ${{ number_format($offer->public_price - $offer->sale_price, 2) }}</span>
                                </div>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center text-xs">
                            @if($offer->sale_deadline)
                                ...
                                @else
                                    <div class="font-bold text-slate-700">{{ \Carbon\Carbon::parse($offer->sale_deadline)->format('d M Y') }}</div>
                                    <div class="text-slate-400">{{ \Carbon\Carbon::parse($offer->sale_deadline)->format('h:i A') }}</div>
                                @endif
                            @else
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <button type="button" 
                                        wire:click="$dispatch('edit-offer', { productId: {{ $offer->id }} })"
